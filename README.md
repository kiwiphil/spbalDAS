...
`DAS()` returns a matrix: each row is a candidate sample of size `n` (1-based population indices). The first `k` columns of any row are a size-`k` DAS sample.

Doubly balanced samples (MATLAB `doubleDAS`) mix spatial spread with balance on auxiliary variables:
...
| --- | --- | --- |
| `J1` | `min(200, floor(N/2))` | Candidate samples. Each step solves a \(J \times J\) assignment (\(O(J^3)\)) and you do this about `n` times. |
| `cache_W` | `N <= 2500` | Cached \(N \times N\) weights. For large `N` (thousands+) use `FALSE`; random access to an 800 MiB matrix is slower than computing distances. |
...
could still hang around step 8–9 on real DAS runs even after a tie-break.
...
Robertson, B. L., Price, C. J. and Reale, M. (2024). Well-spread samples with dynamic sample sizes. *Biometrics* 80(2), ujae026.
